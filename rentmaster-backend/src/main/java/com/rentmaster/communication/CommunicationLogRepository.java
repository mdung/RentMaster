...
public interface CommunicationLogRepository extends JpaRepository<CommunicationLog, Long> {
    ...
    List<CommunicationLog> findByRecipientIdAndRecipientType(Long recipientId, CommunicationLog.RecipientType recipientType);
    
    List<CommunicationLog> findByStatus(CommunicationLog.CommunicationStatus status);
    
    @Query("SELECT COUNT(cl) FROM CommunicationLog cl WHERE DATE(cl.createdAt) = CURRENT_DATE")
